...
// ─── UNIFIED BLACK THEME (all 3 dashboards) ──────────────────────────────────
const BLACK = {
  bg: "#080A0F",
  sidebar: "#04060A",
  card: "#0E1117",
  ...
  // Role-specific accents only
  student: {
    accent: "#22D3A5",
    accentLight: "rgba(34,211,165,0.10)",
    gradient: "linear-gradient(135deg,#22D3A5,#0EA880)",
  },
  tutor: {
    accent: "#F4A228",
    accentLight: "rgba(244,162,40,0.10)",
...
// Build role-specific full theme objects
...
const T = {
  student: makeTheme("student"),
  tutor: makeTheme("tutor"),
  admin: makeTheme("admin"),
};

const F = { display: "'Fraunces',serif", body: "'DM Sans',sans-serif", mono: "'JetBrains Mono',monospace" };

// ─── SHARED UTILITIES ────────────────────────────────────────────────────────
const Avatar = ({ initials, size = 40, gradient = BLACK.student.gradient }: { initials: string; size?: number; gradient?: string }) => (
...
const StarRow = ({ rating, size = 13 }: { rating: number; size?: number }) => (
  <span style={{ color: "#F59E0B", fontSize: size }}>
    ...
    <span style={{ color: "#4A5168", marginLeft: 4, fontFamily: F.body, fontSize: size - 1 }}>{rating.toFixed(1)}</span>
  </span>
);
    ...
    <div style={{ display: "flex", gap: 4 }}>
      {[1, 2, 3, 4, 5].map(n => (
        <span key={n} onMouseEnter={() => setHov(n)} onMouseLeave={() => setHov(0)} onClick={() => onChange(n)}
          style={{ fontSize: 28, cursor: "pointer", color: n <= (hov || value) ? "#F59E0B" : "#2A2D3A", transition: "color .1s" }}>★</span>
      ))}
    </div>
  );
};
...
export default function App() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (td) setTutors(td.map((t: any) => ({
        ...
        reviewCount: t.review_count || 0, verified: t.verified || "pending", earnings: t.earnings || 0,
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: BLACK.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" }}>
        <div style={{ width: 48, height: 48, borderRadius: 14, background: BLACK.admin.gradient, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 900, fontSize: 22, fontFamily: F.display, margin: "0 auto 16px" }}>K</div>
        <p style={{ color: BLACK.textSub, fontFamily: F.body, fontSize: 14 }}>Loading KitsoLink...</p>
      </div>
    </div>
  );
  ...
  if (screen === "login") return (<><FontLink /><LoginPage role={loginRole} onLogin={handleLogin} onBack={() => setScreen("landing")} tutors={tutors} students={students} setTutors={setTutors} setStudents={setStudents} /></>);
...
const TYPE_COLORS: Record<string, { bg: string; color: string }> = {
  "Notes": { bg: "rgba(59,130,246,0.12)", color: "#60A5FA" },
  "Exam Paper": { bg: "rgba(239,68,68,0.12)", color: "#F87171" },
  "Summary": { bg: "rgba(34,211,165,0.12)", color: "#22D3A5" },
  "Textbook": { bg: "rgba(167,139,250,0.12)", color: "#A78BFA" },
};
...
function PublicExplorePage({ onGetStarted, onLogin }: { onGetStarted: () => void; onLogin: (role: Role) => void }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080A0F", fontFamily: F.body }}>
      {/* NAV */}
      <nav style={{ background: "rgba(8,10,15,0.95)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(255,255,255,0.07)", padding: "0 32px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: BLACK.admin.gradient, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 900, fontSize: 16, fontFamily: F.display }}>K</div>
          <span style={{ fontFamily: F.display, fontSize: 22, fontWeight: 900, background: BLACK.admin.gradient, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>KitsoLink</span>
        </div>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <button onClick={() => onLogin("student")} style={{ background: "none", border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: 10, padding: "8px 20px", fontSize: 14, fontWeight: 600, cursor: "pointer", color: BLACK.text, fontFamily: F.body }}>Sign In</button>
          <button onClick={onGetStarted} style={{ background: BLACK.admin.gradient, color: "#fff", border: "none", borderRadius: 10, padding: "9px 22px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: F.body }}>Get Started</button>
        </div>
      </nav>

      {/* HERO */}
      <div style={{ background: "linear-gradient(135deg,#080A0F 0%,#0D1529 55%,#0F1F3A 100%)", padding: "64px 32px 56px", textAlign: "center", position: "relative", overflow: "hidden", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        <div style={{ position: "absolute", top: -150, left: "50%", transform: "translateX(-50%)", width: 600, height: 400, background: "radial-gradient(ellipse,rgba(59,130,246,0.12) 0%,transparent 70%)", pointerEvents: "none" }} />
        <div style={{ position: "relative", zIndex: 1 }}>
          <div style={{ display: "inline-block", background: "rgba(59,130,246,0.15)", border: "1px solid rgba(59,130,246,0.3)", borderRadius: 20, padding: "5px 16px", fontSize: 12, fontWeight: 700, color: "#60A5FA", marginBottom: 18, letterSpacing: "0.08em", textTransform: "uppercase" }}>Built for Botswana Students</div>
          <h1 style={{ fontFamily: F.display, fontSize: "clamp(32px,5vw,58px)", fontWeight: 900, color: BLACK.text, margin: "0 0 14px", lineHeight: 1.1, letterSpacing: "-0.02em" }}>Botswana's Student<br />Resource Platform</h1>
          <p style={{ color: BLACK.textSub, fontSize: 17, maxWidth: 520, margin: "0 auto 32px", lineHeight: 1.6 }}>Browse notes, exam papers, summaries and textbooks from every major university in Botswana.</p>
          <div style={{ maxWidth: 540, margin: "0 auto", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, display: "flex", alignItems: "center", padding: "6px 6px 6px 18px" }}>
            <svg width="18" height="18" fill="none" stroke={BLACK.textMuted} strokeWidth="2" viewBox="0 0 24 24" style={{ flexShrink: 0, marginRight: 10 }}><circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" /></svg>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search notes, modules, exam papers..." style={{ flex: 1, border: "none", outline: "none", fontSize: 15, color: BLACK.text, background: "transparent", fontFamily: F.body }} />
            <button style={{ background: BLACK.admin.gradient, color: "#fff", border: "none", borderRadius: 10, padding: "10px 22px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: F.body }}>Search</button>
          </div>
          <div style={{ display: "flex", justifyContent: "center", gap: 40, marginTop: 36, flexWrap: "wrap" }}>
            {[["500+", "Study Materials"], ["7", "Universities"], ["50+", "Modules"], ["Free", "To Browse"]].map(([n, l]) => (
              <div key={l} style={{ textAlign: "center" }}>
                <div style={{ fontFamily: F.display, fontSize: 26, fontWeight: 900, color: BLACK.text }}>{n}</div>
                <div style={{ color: BLACK.textMuted, fontSize: 13, marginTop: 2 }}>{l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <div style={{ marginBottom: 40 }}>
          <h2 style={{ fontFamily: F.display, fontSize: 22, fontWeight: 700, color: BLACK.text, margin: "0 0 16px" }}>Browse by Field of Study</h2>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button onClick={() => { setSelectedField(null); setSelectedModule(null); setSelectedUni(null); }} style={{ padding: "8px 18px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.1)", background: !selectedField && !selectedUni ? BLACK.admin.gradient : "transparent", color: !selectedField && !selectedUni ? "#fff" : BLACK.textSub, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: F.body }}>All</button>
            {FIELDS_OF_STUDY.map(f => (
              <button key={f} onClick={() => { setSelectedField(f); setSelectedModule(null); setSelectedUni(null); }} style={{ padding: "8px 18px", borderRadius: 20, border: selectedField === f ? "none" : "1px solid rgba(255,255,255,0.08)", background: selectedField === f ? BLACK.admin.gradient : "transparent", color: selectedField === f ? "#fff" : BLACK.textSub, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: F.body, transition: "all 0.15s" }}>{f}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,0.07)", marginBottom: 32 }}>
          {(["universities", "fields"] as const).map(t => (
            <button key={t} onClick={() => { setActiveTab(t); setSelectedUni(null); setSelectedField(null); setSelectedModule(null); }} style={{ padding: "12px 24px", background: "none", border: "none", borderBottom: activeTab === t ? "2px solid #3B82F6" : "2px solid transparent", marginBottom: -1, color: activeTab === t ? "#3B82F6" : BLACK.textSub, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: F.body, textTransform: "capitalize" }}>{t === "universities" ? "By University" : "By Field of Study"}</button>
          ))}
        </div>
        ...
        {(selectedUni || selectedField || selectedModule) && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 24, flexWrap: "wrap" }}>
            ...
            {selectedUni && <><span style={{ color: BLACK.textMuted }}>/</span><button onClick={() => setSelectedModule(null)} style={{ background: "none", border: "none", color: selectedModule ? "#3B82F6" : BLACK.text, fontSize: 13, cursor: "pointer", fontWeight: 600, fontFamily: F.body, padding: 0 }}>{selectedUni}</button></>}
            {selectedField && !selectedUni && <><span style={{ color: BLACK.textMuted }}>/</span><button onClick={() => setSelectedModule(null)} style={{ background: "none", border: "none", color: selectedModule ? "#3B82F6" : BLACK.text, fontSize: 13, cursor: "pointer", fontWeight: 600, fontFamily: F.body, padding: 0 }}>{selectedField}</button></>}
            {selectedModule && <><span style={{ color: BLACK.textMuted }}>/</span><span style={{ color: BLACK.text, fontSize: 13, fontWeight: 600, fontFamily: F.body }}>{selectedModule}</span></>}
          </div>
        )}
        ...
        {activeTab === "universities" && !selectedUni && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 20 }}>
            {UNIVERSITIES.map(u => <UniCard key={u.name} uni={u} onClick={() => setSelectedUni(u.name)} />)}
          </div>
        )}

        {activeTab === "universities" && selectedUni && !selectedModule && (
          <div>
            <h2 style={{ fontFamily: F.display, fontSize: 26, fontWeight: 900, color: BLACK.text, margin: "0 0 6px" }}>{selectedUni}</h2>
            <p style={{ color: BLACK.textSub, fontSize: 14, margin: "0 0 28px" }}>Select a field of study to browse modules and materials</p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 16 }}>
              {uni?.fields.map(field => <FieldCard key={field.name} name={field.name} modules={field.modules} onClick={() => setSelectedField(field.name)} onModuleClick={(mod) => { setSelectedField(field.name); setSelectedModule(mod); }} />)}
            </div>
          </div>
        )}
        ...
        {activeTab === "fields" && !selectedField && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 16 }}>
            {FIELDS_OF_STUDY.map(f => {
              ...
              return (
                <div key={f} onClick={() => setSelectedField(f)} style={{ background: BLACK.card, borderRadius: 16, padding: "22px 20px", border: "1px solid rgba(255,255,255,0.07)", cursor: "pointer", transition: "all 0.2s" }}
                  ...
                  <h3 style={{ fontFamily: F.display, fontSize: 16, fontWeight: 700, color: BLACK.text, margin: "0 0 6px" }}>{f}</h3>
                  <p style={{ color: BLACK.textSub, fontSize: 13, margin: 0 }}>{count} material{count !== 1 ? "s" : ""} available</p>
                </div>
              );
            })}
          </div>
        ...
        {selectedModule && (
          <MaterialsView freeMaterials={freeMaterials} premiumMaterials={premiumMaterials} selectedModule={selectedModule}
            selectedMaterial={selectedMaterial} setSelectedMaterial={setSelectedMaterial}
            onPremium={() => setShowPremiumModal(true)} onLogin={onLogin} />
        )}
      </div>

      {showPremiumModal && <PremiumModal onClose={() => setShowPremiumModal(false)} onSignUp={() => { setShowPremiumModal(false); onGetStarted(); }} />}

      <div style={{ background: BLACK.sidebar, borderTop: "1px solid rgba(255,255,255,0.07)", padding: "40px 32px", textAlign: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, justifyContent: "center", marginBottom: 12 }}>
          <div style={{ width: 30, height: 30, borderRadius: 8, background: BLACK.admin.gradient, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 900, fontFamily: F.display }}>K</div>
          <span style={{ fontFamily: F.display, fontSize: 18, fontWeight: 900, color: BLACK.text }}>KitsoLink</span>
        </div>
        <p style={{ color: BLACK.textMuted, fontSize: 13, margin: 0 }}>Built for Botswana · KitsoLink 2026</p>
      </div>
    </div>
  );
}
...
function UniCard({ uni, onClick }: { uni: typeof UNIVERSITIES[0]; onClick: () => void }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={onClick}
      style={{ background: hov ? BLACK.cardHover : BLACK.card, borderRadius: 18, padding: "24px", border: hov ? "1px solid rgba(59,130,246,0.4)" : "1px solid rgba(255,255,255,0.07)", cursor: "pointer", transition: "all 0.2s", transform: hov ? "translateY(-4px)" : "none" }}>
      <div style={{ width: 48, height: 48, borderRadius: 14, background: "rgba(59,130,246,0.12)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
      ...
      <h3 style={{ fontFamily: F.display, fontSize: 16, fontWeight: 700, color: BLACK.text, margin: "0 0 4px", lineHeight: 1.3 }}>{uni.name}</h3>
      <p style={{ color: BLACK.textSub, fontSize: 13, margin: "0 0 14px" }}>{uni.location} · {uni.fields.length} fields</p>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
        {uni.fields.slice(0, 2).map(f => <span key={f.name} style={{ background: "rgba(59,130,246,0.12)", color: "#60A5FA", fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 20 }}>{f.name}</span>)}
        {uni.fields.length > 2 && <span style={{ background: "rgba(255,255,255,0.05)", color: BLACK.textSub, fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 20 }}>+{uni.fields.length - 2} more</span>}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid rgba(255,255,255,0.07)", paddingTop: 12 }}>
        <span style={{ color: BLACK.textMuted, fontSize: 12 }}>{matCount} materials</span>
...
function FieldCard({ name, modules, onClick, onModuleClick }: { name: string; modules: string[]; onClick: () => void; onModuleClick: (m: string) => void }) {
  return (
    <div style={{ background: BLACK.card, borderRadius: 16, padding: "20px", border: "1px solid rgba(255,255,255,0.07)", transition: "all 0.2s" }}>
      <h3 onClick={onClick} style={{ fontFamily: F.display, fontSize: 16, fontWeight: 700, color: BLACK.text, margin: "0 0 14px", cursor: "pointer" }}>{name}</h3>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {modules.map(mod => {
          const count = SAMPLE_MATERIALS.filter(m => m.module === mod).length;
          return (
            <div key={mod} onClick={() => onModuleClick(mod)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", borderRadius: 8, background: "rgba(255,255,255,0.03)", cursor: "pointer", transition: "background 0.15s" }}
              onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.background = "rgba(59,130,246,0.1)"}
              onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.background = "rgba(255,255,255,0.03)"}>
              <span style={{ fontSize: 13, color: BLACK.text, fontWeight: 500 }}>{mod}</span>
              <span style={{ fontSize: 11, color: BLACK.textMuted }}>{count > 0 ? `${count} file${count > 1 ? "s" : ""}` : "—"}</span>
            </div>
          );
        })}
      </div>
...
function FieldModulesView({ field, onModuleClick }: { field: string; onModuleClick: (m: string) => void }) {
  ...
  return (
    <div>
      <h2 style={{ fontFamily: F.display, fontSize: 24, fontWeight: 900, color: BLACK.text, margin: "0 0 6px" }}>{field}</h2>
      <p style={{ color: BLACK.textSub, fontSize: 14, margin: "0 0 24px" }}>Select a module to see available materials</p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 12 }}>
        {unique.map(mod => {
          const count = SAMPLE_MATERIALS.filter(m => m.module === mod).length;
          return (
            <div key={mod} onClick={() => onModuleClick(mod)} style={{ background: BLACK.card, borderRadius: 12, padding: "16px 18px", border: "1px solid rgba(255,255,255,0.07)", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", transition: "all 0.15s" }}
              onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(59,130,246,0.4)"; (e.currentTarget as HTMLDivElement).style.background = "rgba(59,130,246,0.06)"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(255,255,255,0.07)"; (e.currentTarget as HTMLDivElement).style.background = BLACK.card; }}>
              <div>
                <p style={{ margin: "0 0 3px", fontSize: 14, fontWeight: 600, color: BLACK.text }}>{mod}</p>
                <p style={{ margin: 0, fontSize: 12, color: BLACK.textMuted }}>{count} material{count !== 1 ? "s" : ""}</p>
              </div>
...
function MaterialsView({ freeMaterials, premiumMaterials, selectedModule, selectedMaterial, setSelectedMaterial, onPremium, onLogin }: {
  ...
  if (selectedMaterial) return <MaterialReader material={selectedMaterial} onBack={() => setSelectedMaterial(null)} onSignUp={() => onLogin("student")} />;
  const shown = tab === "free" ? freeMaterials : premiumMaterials;
  return (
    <div>
      <h2 style={{ fontFamily: F.display, fontSize: 24, fontWeight: 900, color: BLACK.text, margin: "0 0 6px" }}>{selectedModule}</h2>
      <p style={{ color: BLACK.textSub, fontSize: 14, margin: "0 0 24px" }}>{freeMaterials.length} free · {premiumMaterials.length} premium materials</p>
      <div style={{ display: "flex", gap: 0, background: "rgba(255,255,255,0.04)", borderRadius: 12, padding: 4, marginBottom: 24, width: "fit-content" }}>
        <button onClick={() => setTab("free")} style={{ padding: "8px 24px", borderRadius: 9, border: "none", background: tab === "free" ? BLACK.card : "transparent", color: tab === "free" ? BLACK.text : BLACK.textSub, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: F.body, transition: "all 0.15s" }}>Free ({freeMaterials.length})</button>
        <button onClick={() => setTab("premium")} style={{ padding: "8px 24px", borderRadius: 9, border: "none", background: tab === "premium" ? BLACK.card : "transparent", color: tab === "premium" ? BLACK.warning : BLACK.textSub, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: F.body, transition: "all 0.15s" }}>Premium ({premiumMaterials.length})</button>
      </div>
      {tab === "premium" && (
        <div style={{ background: "rgba(240,160,32,0.08)", border: "1px solid rgba(240,160,32,0.2)", borderRadius: 14, padding: "16px 20px", marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <div>
            <p style={{ margin: "0 0 3px", fontWeight: 700, fontSize: 15, color: BLACK.warning }}>Premium Materials</p>
            <p style={{ margin: 0, fontSize: 13, color: BLACK.textSub }}>Subscribe to unlock all premium notes, past exam papers and textbooks.</p>
          </div>
          <button onClick={onPremium} style={{ background: BLACK.admin.gradient, color: "#fff", border: "none", borderRadius: 10, padding: "10px 22px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: F.body }}>Subscribe Now</button>
        </div>
      )}
      {shown.length === 0 ? (
        <div style={{ textAlign: "center", padding: "48px 0" }}><p style={{ color: BLACK.textMuted, fontSize: 15 }}>No {tab} materials for this module yet.</p></div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 18 }}>
          {shown.map(mat => <MaterialCard key={mat.id} material={mat} onOpen={() => { if (mat.premium) onPremium(); else setSelectedMaterial(mat); }} />)}
        </div>
      )}
    </div>
  );
...
function MaterialCard({ material: m, onOpen }: { material: Material; onOpen: () => void }) {
  const [hov, setHov] = useState(false);
  const tc = TYPE_COLORS[m.type] || { bg: "rgba(255,255,255,0.05)", color: BLACK.textSub };
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={onOpen}
      style={{ background: hov ? BLACK.cardHover : BLACK.card, borderRadius: 16, padding: "20px", border: hov ? "1px solid rgba(59,130,246,0.3)" : "1px solid rgba(255,255,255,0.07)", cursor: "pointer", transition: "all 0.2s", transform: hov ? "translateY(-3px)" : "none", position: "relative" }}>
      {m.premium && <div style={{ position: "absolute", top: 14, right: 14, background: "rgba(240,160,32,0.12)", color: BLACK.warning, fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 20 }}>Premium</div>}
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        ...
        <span style={{ color: BLACK.textMuted, fontSize: 12, alignSelf: "center" }}>{m.year}</span>
      </div>
      <h3 style={{ fontFamily: F.display, fontSize: 15, fontWeight: 700, color: BLACK.text, margin: "0 0 8px", lineHeight: 1.4, paddingRight: m.premium ? 52 : 0 }}>{m.title}</h3>
      <p style={{ fontSize: 12, color: BLACK.textSub, margin: "0 0 14px", lineHeight: 1.5 }}>{m.preview.slice(0, 80)}...</p>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 12 }}>
        <div style={{ display: "flex", gap: 2 }}>
          {[1, 2, 3, 4, 5].map(n => <span key={n} style={{ color: n <= Math.round(m.rating) ? "#F59E0B" : "#2A2D3A", fontSize: 12 }}>★</span>)}
          <span style={{ color: BLACK.textMuted, fontSize: 12, marginLeft: 4 }}>{m.rating}</span>
        </div>
        <span style={{ color: BLACK.textMuted, fontSize: 12 }}>{m.pages} pages · {m.downloads.toLocaleString()} views</span>
      </div>
    </div>
  );
}
...
function MaterialReader({ material: m, onBack, onSignUp }: { material: Material; onBack: () => void; onSignUp: () => void }) {
  ...
  return (
    <div>
      ...
      <div style={{ background: BLACK.card, borderRadius: 20, border: "1px solid rgba(255,255,255,0.07)", overflow: "hidden" }}>
        <div style={{ background: "linear-gradient(135deg,#0D1529,#1E3A8A)", padding: "28px 28px 24px" }}>
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            <span style={{ background: "rgba(255,255,255,0.15)", color: "#fff", fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20 }}>{m.type}</span>
            <span style={{ background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.6)", fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 20 }}>{m.year}</span>
          </div>
          <h2 style={{ fontFamily: F.display, fontSize: 22, fontWeight: 900, color: "#fff", margin: "0 0 8px" }}>{m.title}</h2>
          <p style={{ color: "rgba(255,255,255,0.55)", fontSize: 13, margin: "0 0 10px" }}>{m.university} · {m.module}</p>
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
            {[["Pages", String(m.pages)], ["Views", m.downloads.toLocaleString()], ["Rating", m.rating.toFixed(1)]].map(([l, v]) => (
              <div key={l}><p style={{ margin: "0 0 2px", color: "rgba(255,255,255,0.4)", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.06em" }}>{l}</p><p style={{ margin: 0, color: "#fff", fontWeight: 700, fontSize: 15, fontFamily: F.mono }}>{v}</p></div>
            ))}
          </div>
        </div>
        <div style={{ padding: "24px 28px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <h4 style={{ margin: 0, fontSize: 13, fontWeight: 700, color: BLACK.textSub, textTransform: "uppercase", letterSpacing: "0.06em" }}>Document Preview — {freePages} of {m.pages} pages</h4>
            <span style={{ background: "rgba(59,130,246,0.12)", color: "#60A5FA", fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20 }}>Free Preview</span>
          </div>
          {Array.from({ length: freePages }, (_, i) => (
            <div key={i} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "16px 20px", marginBottom: 12 }}>
              <p style={{ margin: "0 0 10px", fontSize: 11, fontWeight: 700, color: BLACK.textMuted, textTransform: "uppercase" }}>Page {i + 1}</p>
              {i === 0 && <div style={{ background: "rgba(59,130,246,0.08)", borderLeft: "3px solid #3B82F6", borderRadius: "0 8px 8px 0", padding: "10px 14px", marginBottom: 10 }}><p style={{ margin: 0, fontSize: 13, color: "#60A5FA", lineHeight: 1.6 }}><strong>Topics covered: </strong>{m.preview}</p></div>}
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {[0.9, 0.75, 0.85, 0.6, 0.8].map((w, j) => <div key={j} style={{ height: 9, background: "rgba(255,255,255,0.06)", borderRadius: 4, width: `${w * 100}%` }} />)}
              </div>
            </div>
          ))}
          <div style={{ background: "rgba(240,160,32,0.08)", border: "1px solid rgba(240,160,32,0.2)", borderRadius: 16, padding: "28px 24px", textAlign: "center", marginTop: 8 }}>
            <h3 style={{ fontFamily: F.display, fontSize: 20, fontWeight: 900, color: BLACK.warning, margin: "0 0 8px" }}>Full Document Locked</h3>
            <p style={{ color: BLACK.textSub, fontSize: 14, margin: "0 0 20px", lineHeight: 1.6 }}>Sign up for free to access full documents, or subscribe for premium materials.</p>
            <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
              <button onClick={onSignUp} style={{ background: BLACK.admin.gradient, color: "#fff", border: "none", borderRadius: 10, padding: "11px 24px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: F.body }}>Sign Up Free</button>
              <button onClick={onSignUp} style={{ background: "rgba(240,160,32,0.15)", color: BLACK.warning, border: "1px solid rgba(240,160,32,0.3)", borderRadius: 10, padding: "11px 24px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: F.body }}>Subscribe for Premium</button>
            </div>
          </div>
        </div>
      </div>
...
function PremiumModal({ onClose, onSignUp }: { onClose: () => void; onSignUp: () => void }) {
  const plans = [
    { name: "Monthly", price: "P89", period: "/month", color: "#3B82F6" },
    { name: "Per Semester", price: "P199", period: "/6 months", saves: "Save 55%", color: "#8B5CF6", popular: true },
    { name: "Annual", price: "P299", period: "/year", saves: "Save 72%", color: "#22D3A5" },
  ];
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={onClose}>
      <div style={{ background: BLACK.card, border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, width: "100%", maxWidth: 640, overflow: "hidden" }} onClick={e => e.stopPropagation()}>
        <div style={{ background: "linear-gradient(135deg,#0D1529,#1E3A8A)", padding: "28px 28px 24px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div><p style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.06em" }}>KitsoLink</p><h2 style={{ fontFamily: F.display, fontSize: 24, fontWeight: 900, color: "#fff", margin: 0 }}>Unlock Premium Materials</h2></div>
          <button onClick={onClose} style={{ background: "rgba(255,255,255,0.1)", border: "none", borderRadius: 8, width: 34, height: 34, fontSize: 16, cursor: "pointer", color: "#fff" }}>×</button>
        </div>
        <div style={{ padding: "24px 28px 28px" }}>
          <p style={{ color: BLACK.textSub, fontSize: 14, margin: "0 0 24px", lineHeight: 1.6 }}>Get unlimited access to all premium notes, past exam papers, textbooks and more.</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 24 }}>
            {plans.map(p => (
              <div key={p.name} style={{ borderRadius: 16, padding: "20px 16px", border: p.popular ? `2px solid ${p.color}` : "1px solid rgba(255,255,255,0.1)", position: "relative", textAlign: "center", background: p.popular ? `${p.color}10` : "transparent" }}>
                ...
                {p.saves && <div style={{ background: "rgba(255,255,255,0.1)", color: BLACK.text, borderRadius: 20, padding: "2px 10px", fontSize: 11, fontWeight: 700, marginBottom: 8, display: "inline-block" }}>{p.saves}</div>}
                <h4 style={{ fontFamily: F.display, fontSize: 15, color: BLACK.text, margin: "0 0 8px" }}>{p.name}</h4>
                <div style={{ display: "flex", alignItems: "baseline", justifyContent: "center", gap: 2, marginBottom: 16 }}>
                  ...
                  <span style={{ color: BLACK.textMuted, fontSize: 12 }}>{p.period}</span>
                </div>
                <button onClick={onSignUp} style={{ width: "100%", padding: "9px", borderRadius: 10, border: "none", background: p.color, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: F.body }}>Subscribe</button>
              </div>
            ))}
          </div>
          <p style={{ textAlign: "center", color: BLACK.textMuted, fontSize: 12, margin: 0 }}>Secure payment · Cancel anytime · Instant access</p>
        </div>
      </div>
    </div>
  );
...
function LandingPage({ onChooseRole, onBack }: { onChooseRole: (r: Role) => void; onBack?: () => void }) {
  const roles: { role: Role; icon: string; title: string; desc: string; gradient: string }[] = [
    { role: "student", icon: "🎓", title: "Student", desc: "Browse tutors, book sessions & track your learning journey.", gradient: BLACK.student.gradient },
    { role: "tutor", icon: "👨‍🏫", title: "Tutor", desc: "Manage bookings, earn money & grow your student base.", gradient: BLACK.tutor.gradient },
    { role: "admin", icon: "⚙️", title: "Administrator", desc: "Monitor the platform, verify tutors & manage accounts.", gradient: BLACK.admin.gradient },
  ];
  return (
    <div style={{ minHeight: "100vh", background: BLACK.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 40, fontFamily: F.body, position: "relative", overflow: "hidden" }}>
      <div style={{ position: "fixed", top: -200, left: -200, width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(circle,rgba(34,211,165,0.06) 0%,transparent 70%)", pointerEvents: "none" }} />
      ...
      <div style={{ textAlign: "center", marginBottom: 56, position: "relative" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          <div style={{ width: 44, height: 44, borderRadius: 14, background: BLACK.admin.gradient, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>K</div>
          <span style={{ fontFamily: F.display, fontSize: 32, fontWeight: 900, color: BLACK.text, letterSpacing: "-0.02em" }}>KitsoLink</span>
        </div>
        <h1 style={{ fontFamily: F.display, fontSize: "clamp(36px,5vw,64px)", fontWeight: 900, color: BLACK.text, margin: "0 0 16px", lineHeight: 1.1, letterSpacing: "-0.03em" }}>
          Connecting Botswana's<br /><span style={{ background: BLACK.student.gradient, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Students & Tutors</span>
        </h1>
        <p style={{ color: BLACK.textSub, fontSize: 18, maxWidth: 480, margin: "0 auto", lineHeight: 1.6 }}>Choose your role to enter your personalised dashboard</p>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 20, width: "100%", maxWidth: 900 }}>
        {roles.map(r => <RoleCard key={r.role} {...r} onClick={() => onChooseRole(r.role)} />)}
      </div>
      <div style={{ display: "flex", gap: 20, alignItems: "center", marginTop: 40 }}>
        {onBack && <button onClick={onBack} style={{ background: "none", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "7px 16px", color: BLACK.textSub, fontSize: 13, cursor: "pointer", fontFamily: F.body }}>← Back to Explore</button>}
        <p style={{ color: BLACK.textMuted, fontSize: 13, margin: 0 }}>Built for Botswana · KitsoLink 2026</p>
      </div>
    </div>
  );
...
function RoleCard({ icon, title, desc, gradient, onClick }: { icon: string; title: string; desc: string; gradient: string; onClick: () => void }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={onClick}
      style={{ background: hov ? BLACK.cardHover : BLACK.card, border: hov ? "1px solid rgba(255,255,255,0.12)" : "1px solid rgba(255,255,255,0.07)", borderRadius: 20, padding: "36px 28px", cursor: "pointer", transition: "all 0.25s", transform: hov ? "translateY(-6px)" : "none" }}>
      ...
      <h3 style={{ fontFamily: F.display, fontSize: 22, fontWeight: 700, color: BLACK.text, margin: "0 0 10px" }}>{title}</h3>
      <p style={{ color: BLACK.textSub, fontSize: 14, margin: "0 0 24px", lineHeight: 1.6 }}>{desc}</p>
...
function LoginPage({ role, onLogin, onBack, tutors, students, setTutors, setStudents }: {
  ...
  const roleColors = { student: BLACK.student, tutor: BLACK.tutor, admin: BLACK.admin };
  const rc = roleColors[role];
  const roleIcon = role === "student" ? "S" : role === "tutor" ? "T" : "A";
  ...
  const inp: React.CSSProperties = { width: "100%", padding: "12px 14px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: BLACK.text, fontSize: 14, fontFamily: F.body, outline: "none", boxSizing: "border-box" };
  const lbl: React.CSSProperties = { display: "block", fontSize: 11, fontWeight: 600, color: BLACK.textSub, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.07em", fontFamily: F.body };

  return (
    <div style={{ minHeight: "100vh", background: BLACK.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: BLACK.card, border: "1px solid rgba(255,255,255,0.08)", borderRadius: 24, padding: "44px 40px", width: "100%", maxWidth: 480, position: "relative" }}>
        <button onClick={onBack} style={{ position: "absolute", top: 20, left: 20, background: "none", border: "none", color: BLACK.textSub, cursor: "pointer", fontSize: 13, fontFamily: F.body }}>← Back</button>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ width: 56, height: 56, borderRadius: 16, background: `${rc.accent}18`, border: `1px solid ${rc.accent}33`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, margin: "0 auto 14px", color: rc.accent, fontWeight: 900, fontFamily: F.display }}>{roleIcon}</div>
          <h2 style={{ fontFamily: F.display, fontSize: 26, fontWeight: 700, color: BLACK.text, margin: "0 0 6px" }}>
          ...
          {role !== "admin" && (
            <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 12 }}>
              {(["login", "register"] as const).map(m => (
                <button key={m} onClick={() => setMode(m)} style={{ padding: "6px 18px", borderRadius: 20, border: "none", background: mode === m ? rc.accent : "transparent", color: mode === m ? "#fff" : BLACK.textSub, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: F.body }}>{m === "login" ? "Sign In" : "Register"}</button>
              ))}
            </div>
          )}
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {role === "admin" ? (
            <>
              ...
              <div style={{ background: `${rc.accent}12`, border: `1px solid ${rc.accent}30`, borderRadius: 10, padding: "10px 14px" }}>
                <p style={{ color: rc.accent, fontSize: 12, margin: 0, fontFamily: F.mono }}>Demo: mark / mark12345</p>
              </div>
            </>
          ) : mode === "login" ? (
            <>
          ...
          {err && <div style={{ background: "rgba(255,92,92,0.1)", border: "1px solid rgba(255,92,92,0.25)", borderRadius: 10, padding: "10px 14px" }}><p style={{ color: BLACK.danger, fontSize: 13, margin: 0, fontFamily: F.body }}>{err}</p></div>}
          {mode === "register" && role !== "admin" && <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: "10px 14px" }}><p style={{ color: BLACK.textSub, fontSize: 12, margin: 0, fontFamily: F.body }}>{role === "tutor" ? "Your profile will be reviewed by admin before appearing to students." : "Check your email to confirm your account before signing in."}</p></div>}
          <button onClick={handleSubmit} disabled={submitting} style={{ background: submitting ? "#1A1D28" : rc.gradient, color: "#fff", border: "none", borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 700, cursor: submitting ? "not-allowed" : "pointer", fontFamily: F.body, marginTop: 4 }}>
...
function Shell({ navItems, activeTab, setActiveTab, user, theme, onLogout, children }: {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: BLACK.bg, fontFamily: F.body }}>
      {/* Sidebar */}
      <aside style={{ width: 240, background: BLACK.sidebar, display: "flex", flexDirection: "column", padding: "24px 14px", position: "sticky", top: 0, height: "100vh", flexShrink: 0, borderRight: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ padding: "0 10px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 10, background: theme.gradient, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, color: "#fff", fontWeight: 900, fontFamily: F.display }}>K</div>
            <span style={{ fontFamily: F.display, fontSize: 18, fontWeight: 900, color: BLACK.text }}>KitsoLink</span>
          </div>
        </div>
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
          {navItems.map(item => {
            ...
            return (
              <button key={item.id} onClick={() => setActiveTab(item.id)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", borderRadius: 10, border: "none", cursor: "pointer", background: active ? theme.accentLight : "transparent", color: active ? theme.accent : "rgba(255,255,255,0.4)", fontSize: 13, fontFamily: F.body, fontWeight: active ? 600 : 400, transition: "all 0.15s", textAlign: "left", width: "100%" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 15 }}>{item.icon}</span>{item.label}
                </div>
                {item.badge != null && item.badge > 0 && <span style={{ background: BLACK.danger, color: "#fff", fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 10, fontFamily: F.mono }}>{item.badge}</span>}
              </button>
            );
          })}
        </nav>
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 6px", marginBottom: 10 }}>
            ...
            <div style={{ flex: 1, overflow: "hidden" }}>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: BLACK.text, fontFamily: F.body, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.name}</p>
              <p style={{ margin: 0, fontSize: 11, color: "rgba(255,255,255,0.25)", fontFamily: F.body, textTransform: "capitalize" }}>{user.role}</p>
            </div>
          </div>
          <button onClick={onLogout} style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, padding: "8px 12px", color: "rgba(255,255,255,0.3)", fontSize: 12, cursor: "pointer", fontFamily: F.body, textAlign: "left" }}>
            Sign Out
          </button>
        </div>
      </aside>
...
function StudentDashboard({ user, ...shared }: { user: User } & SharedProps) {
  ...
  const navItems = [
    ...
    { id: "upload", label: "Upload Material", icon: "📤" },
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 16, marginBottom: 32 }}>
        {[
          ...
          <StatCard key={s.label} label={s.label} value={s.value} icon={s.icon} color={s.color} theme={theme} />
        ))}
      </div>
...
function StudentTutors({ user, tutors, bookings, setBookings, setPayments, reviews, theme }: { user: User; tutors: TutorProfile[]; bookings: Booking[]; setBookings: React.Dispatch<React.SetStateAction<Booking[]>>; setPayments: React.Dispatch<React.SetStateAction<Payment[]>>; reviews: Review[]; theme: AnyTheme }) {
  ...
  const inp: React.CSSProperties = { width: "100%", padding: "11px 14px", borderRadius: 10, border: `1px solid ${theme.border}`, background: "rgba(255,255,255,0.04)", color: theme.text, fontSize: 14, fontFamily: F.body, outline: "none", boxSizing: "border-box" };
    ...
    <div>
      ...
      <div style={{ background: theme.card, borderRadius: 20, border: `1px solid ${theme.border}`, overflow: "hidden", marginBottom: 24 }}>
        <div style={{ background: theme.gradient, padding: "32px 28px", display: "flex", gap: 20, alignItems: "center" }}>
          <Avatar initials={selected.avatar} size={72} gradient="rgba(0,0,0,0.2)" />
          <div>
            <h2 style={{ fontFamily: F.display, fontSize: 26, fontWeight: 900, color: "#fff", margin: "0 0 4px" }}>{selected.name}</h2>
            <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 14, margin: "0 0 8px" }}>{selected.university}</p>
            <StarRow rating={selected.rating} size={15} />
            <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 13, marginLeft: 8 }}>({selected.reviewCount} reviews)</span>
          </div>
          <div style={{ marginLeft: "auto", textAlign: "right" }}>
            <p style={{ fontFamily: F.display, fontSize: 32, fontWeight: 900, color: "#fff", margin: "0 0 4px" }}>P{selected.rate}/hr</p>
            <span style={{ background: "rgba(0,0,0,0.2)", color: "#fff", fontSize: 12, fontWeight: 600, padding: "4px 12px", borderRadius: 20 }}>{selected.available ? "✓ Available" : "Busy"}</span>
          </div>
        </div>
        <div style={{ padding: "28px" }}>
          ...
          {!booked ? (
            <>
              {!bookModal ? (
                <button onClick={() => setBookModal(true)} disabled={!selected.available} style={{ background: selected.available ? theme.gradient : "rgba(255,255,255,0.1)", color: "#fff", border: "none", borderRadius: 12, padding: "13px 28px", fontSize: 15, fontWeight: 700, cursor: selected.available ? "pointer" : "not-allowed", fontFamily: F.body }}>📅 Book a Session</button>
              ) : (
                <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${theme.accent}44`, borderRadius: 16, padding: 24 }}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <button onClick={() => setBookModal(false)} style={{ background: "rgba(255,255,255,0.05)", border: `1px solid ${theme.border}`, color: theme.textSub, borderRadius: 10, padding: "11px 20px", fontSize: 14, cursor: "pointer", fontFamily: F.body }}>Cancel</button>
                  </div>
                </div>
              )}
            </>
          ) : (
            <div style={{ background: "rgba(34,211,165,0.1)", border: "1px solid rgba(34,211,165,0.25)", borderRadius: 14, padding: "20px 24px" }}>
              <p style={{ color: theme.success, fontWeight: 700, margin: "0 0 4px", fontSize: 15 }}>✓ Booking Requested!</p>
              <p style={{ color: theme.textSub, margin: 0, fontSize: 13 }}>Your session request has been sent to {selected.name}. They will confirm shortly.</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div>
      <PageHeader title="Find a Tutor" sub={`${filtered.length} approved tutors available`} theme={theme} />
...
function TutorCard({ tutor, theme, onClick, reviews }: { tutor: TutorProfile; theme: AnyTheme; onClick: () => void; reviews: Review[] }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={onClick}
      style={{ background: hov ? BLACK.cardHover : theme.card, borderRadius: 16, padding: 24, border: hov ? `1px solid ${theme.accent}55` : `1px solid ${theme.border}`, cursor: "pointer", transition: "all 0.2s", transform: hov ? "translateY(-3px)" : "none" }}>
      <div style={{ display: "flex", gap: 14, alignItems: "flex-start", marginBottom: 14 }}>
        ...
        <span style={{ background: tutor.available ? "rgba(34,211,165,0.12)" : "rgba(255,92,92,0.12)", color: tutor.available ? theme.success : theme.danger, fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 20 }}>{tutor.available ? "Available" : "Busy"}</span>
      </div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
        ...
        {tutor.modules.length > 3 && <span style={{ background: "rgba(255,255,255,0.05)", color: theme.textMuted, fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 20 }}>+{tutor.modules.length - 3}</span>}
      </div>
      <div style={{ borderTop: `1px solid ${theme.border}`, paddingTop: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <p style={{ margin: 0, fontSize: 13, color: theme.textSub, lineHeight: 1.4 }}>{(tutor.bio || "No bio provided.").slice(0, 60)}...</p>
...
function StudentBookings({ user, bookings, setBookings, theme }: { user: User; bookings: Booking[]; setBookings: React.Dispatch<React.SetStateAction<Booking[]>>; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="My Bookings" sub={`${bookings.length} total bookings`} theme={theme} />
          ...
          {bookings.map(b => (
            <div key={b.id} style={{ background: theme.card, borderRadius: 16, padding: 20, border: `1px solid ${theme.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <StatusBadge status={b.status} />
                {b.status === "pending" && <button onClick={() => cancel(b.id)} style={{ background: "rgba(255,92,92,0.1)", color: theme.danger, border: "none", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: F.body }}>Cancel</button>}
              </div>
            </div>
          ))}
        </div>
...
function StudentMessages({ user, messages, setMessages, tutors, theme }: { user: User; messages: Message[]; setMessages: React.Dispatch<React.SetStateAction<Message[]>>; tutors: TutorProfile[]; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="Messages" sub="Chat with your tutors" theme={theme} />
      <div style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 20, height: 600 }}>
        ...
        <div style={{ background: theme.card, borderRadius: 16, border: `1px solid ${theme.border}`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          {!activeConv ? (
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}><EmptyState icon="💬" text="Select a tutor to start messaging" /></div>
          ) : (
            <>
                ...
                {convMsgs.map(m => {
                  ...
                  return (
                    <div key={m.id} style={{ display: "flex", justifyContent: mine ? "flex-end" : "flex-start" }}>
                      <div style={{ background: mine ? theme.gradient : "rgba(255,255,255,0.06)", color: mine ? "#fff" : theme.text, borderRadius: mine ? "16px 16px 4px 16px" : "16px 16px 16px 4px", padding: "10px 16px", maxWidth: "70%", fontSize: 14, lineHeight: 1.5 }}>
              ...
              <div style={{ padding: "12px 16px", borderTop: `1px solid ${theme.border}`, display: "flex", gap: 10 }}>
                <input value={newMsg} onChange={e => setNewMsg(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMsg()} placeholder="Type a message..." style={{ flex: 1, padding: "10px 14px", borderRadius: 10, border: `1px solid ${theme.border}`, background: "rgba(255,255,255,0.04)", color: theme.text, fontSize: 14, fontFamily: F.body, outline: "none" }} />
...
function StudentPayments({ user, payments, theme }: { user: User; payments: Payment[]; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="Payments" sub="Your transaction history" theme={theme} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(180px,1fr))", gap: 14, marginBottom: 24 }}>
        ...
        ].map(s => <StatCard key={s.label} label={s.label} value={s.value} icon={s.icon} color={s.color} theme={theme} />)}
      </div>
      <div style={{ background: theme.card, borderRadius: 16, border: `1px solid ${theme.border}`, overflow: "hidden" }}>
        ...
        {payments.length === 0 ? <EmptyState icon="💳" text="No transactions yet" /> : payments.map(p => (
          <div key={p.id} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", padding: "14px 20px", borderBottom: `1px solid ${theme.border}`, alignItems: "center" }}>
            ...
            <StatusPayBadge status={p.status} />
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <PageHeader title="My Reviews" sub={`${reviews.length} reviews written`} theme={theme} inline />
      ...
      {showForm && (
        <div style={{ background: theme.card, border: `1px solid ${theme.accent}44`, borderRadius: 16, padding: 24, marginBottom: 24 }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <select value={selTutor || ""} onChange={e => setSelTutor(e.target.value)} style={inp}>
          ...
          <div style={{ marginBottom: 16 }}><label style={{ display: "block", fontSize: 11, fontWeight: 600, color: theme.textSub, marginBottom: 6, textTransform: "uppercase" }}>Comment</label><textarea value={comment} onChange={e => setComment(e.target.value)} rows={3} style={{ ...inp, resize: "vertical" }} /></div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={submit} style={{ background: theme.gradient, color: "#fff", border: "none", borderRadius: 10, padding: "10px 22px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: F.body }}>Submit Review</button>
            <button onClick={() => setShowForm(false)} style={{ background: "rgba(255,255,255,0.05)", border: `1px solid ${theme.border}`, color: theme.textSub, borderRadius: 10, padding: "10px 20px", fontSize: 14, cursor: "pointer", fontFamily: F.body }}>Cancel</button>
          </div>
        </div>
      )}
...
function TutorDashboard({ user, ...shared }: { user: User } & SharedProps) {
  ...
  return (
    <Shell navItems={navItems} activeTab={tab} setActiveTab={setTab} user={user} theme={th} onLogout={onLogout}>
      ...
      {tab === "upload" && <div style={{ maxWidth: 720, margin: "0 auto" }}><PageHeader title="Upload Material" sub="Share your study materials with students" theme={th} /><MaterialUploadForm user={user} theme={th} onClose={() => setTab("home")} /></div>}
    </Shell>
  );
}
    ...
    <div>
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 12 }}>
          ...
          {profile.verified === "pending" && <Badge label="Verification Pending" color={theme.warning} bg="rgba(240,160,32,0.12)" />}
          {profile.verified === "approved" && <Badge label="✓ Verified" color={theme.success} bg="rgba(34,211,165,0.12)" />}
          {profile.verified === "rejected" && <Badge label="Not Verified" color={theme.danger} bg="rgba(255,92,92,0.12)" />}
        </div>
...
function TutorRequests({ profile, bookings, setBookings, setPayments, theme }: { profile: TutorProfile; bookings: Booking[]; setBookings: React.Dispatch<React.SetStateAction<Booking[]>>; setPayments?: React.Dispatch<React.SetStateAction<Payment[]>>; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="Student Requests" sub={`${pending.length} pending requests`} theme={theme} />
      {pending.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          ...
          {pending.map(b => (
            <div key={b.id} style={{ background: theme.card, borderRadius: 16, padding: 20, border: `1px solid ${theme.accent}33`, marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => updateStatus(b.id, "confirmed")} style={{ background: theme.gradient, color: "#fff", border: "none", borderRadius: 10, padding: "9px 20px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: F.body }}>✓ Accept</button>
                <button onClick={() => updateStatus(b.id, "cancelled")} style={{ background: "rgba(255,92,92,0.1)", color: theme.danger, border: "none", borderRadius: 10, padding: "9px 20px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: F.body }}>✕ Decline</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          {others.map(b => (
            <div key={b.id} style={{ background: theme.card, borderRadius: 14, padding: "16px 20px", border: `1px solid ${theme.border}`, marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <StatusBadge status={b.status} />
                {b.status === "confirmed" && <button onClick={() => updateStatus(b.id, "completed")} style={{ background: "rgba(34,211,165,0.1)", color: theme.success, border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: F.body }}>Mark Complete</button>}
              </div>
            </div>
          ))}
        </div>
...
function TutorSchedule({ profile, bookings, setTutors, theme }: { profile: TutorProfile; bookings: Booking[]; setTutors: React.Dispatch<React.SetStateAction<TutorProfile[]>>; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="Schedule Management" sub="Manage your availability and upcoming sessions" theme={theme} />
      <div style={{ background: theme.card, borderRadius: 16, padding: 24, border: `1px solid ${theme.border}`, marginBottom: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div onClick={toggleAvail} style={{ width: 56, height: 28, borderRadius: 14, background: profile.available ? theme.success : "rgba(255,255,255,0.1)", cursor: "pointer", position: "relative", transition: "background 0.2s" }}>
        ...
        <div style={{ marginTop: 16, background: profile.available ? "rgba(34,211,165,0.08)" : "rgba(255,92,92,0.08)", borderRadius: 10, padding: "12px 16px" }}>
...
function TutorMessages({ user, messages, setMessages, bookings, theme }: { user: User; messages: Message[]; setMessages: React.Dispatch<React.SetStateAction<Message[]>>; bookings: Booking[]; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="Messages" sub="Chat with your students" theme={theme} />
            ...
            <>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 10 }}>
                {convMsgs.map(m => {
                  ...
                  return (
                    <div key={m.id} style={{ display: "flex", justifyContent: mine ? "flex-end" : "flex-start" }}>
                      <div style={{ background: mine ? theme.gradient : "rgba(255,255,255,0.06)", color: mine ? "#fff" : theme.text, borderRadius: mine ? "16px 16px 4px 16px" : "16px 16px 16px 4px", padding: "10px 16px", maxWidth: "70%" }}>
              ...
              <div style={{ padding: "12px 16px", borderTop: `1px solid ${theme.border}`, display: "flex", gap: 10 }}>
                <input value={newMsg} onChange={e => setNewMsg(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMsg()} placeholder="Type a message..." style={{ flex: 1, padding: "10px 14px", borderRadius: 10, border: `1px solid ${theme.border}`, background: "rgba(255,255,255,0.04)", color: theme.text, fontSize: 14, fontFamily: F.body, outline: "none" }} />
                <button onClick={sendMsg} style={{ background: theme.gradient, color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontWeight: 700, cursor: "pointer", fontFamily: F.body }}>Send</button>
              </div>
            </>
          )}
        </div>
...
function TutorEarnings({ profile, bookings, payments, theme }: { profile: TutorProfile; bookings: Booking[]; payments: Payment[]; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="Earnings" sub="Track your income from tutoring sessions" theme={theme} />
...
function TutorReviewsTab({ reviews, theme }: { reviews: Review[]; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="My Reviews" sub={`${reviews.length} reviews from students`} theme={theme} />
      {reviews.length > 0 && (
...
function TutorProfileTab({ profile, setTutors, theme }: { profile: TutorProfile; setTutors: React.Dispatch<React.SetStateAction<TutorProfile[]>>; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="My Profile" sub="Update your tutor profile" theme={theme} />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
      ...
      <div style={{ marginTop: 20, display: "flex", gap: 12, alignItems: "center" }}>
        <button onClick={save} style={{ background: theme.gradient, color: "#fff", border: "none", borderRadius: 12, padding: "12px 28px", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: F.body }}>Save Changes</button>
...
function AdminOverview({ tutors, students, bookings, reviews, payments, theme }: { tutors: TutorProfile[]; students: StudentProfile[]; bookings: Booking[]; reviews: Review[]; payments: Payment[]; theme: AnyTheme }) {
  ...
  const stats = [
    ...
    { label: "Total Tutors", value: tutors.length, icon: "👨‍🏫", color: BLACK.success },
    { label: "Total Bookings", value: bookings.length, icon: "📅", color: BLACK.warning },
    { label: "Platform Revenue", value: `P${totalRevenue}`, icon: "💰", color: "#8B5CF6" },
    { label: "Total Reviews", value: reviews.length, icon: "⭐", color: BLACK.warning },
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 16, marginBottom: 32 }}>
        {stats.map(s => <StatCard key={s.label} label={s.label} value={s.value} icon={s.icon} color={s.color} theme={theme} />)}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div style={{ background: theme.card, borderRadius: 16, padding: 24, border: `1px solid ${theme.border}` }}>
          <h3 style={{ fontFamily: F.display, fontSize: 18, color: theme.text, margin: "0 0 16px" }}>Recent Bookings</h3>
        ...
        <div style={{ background: theme.card, borderRadius: 16, padding: 24, border: `1px solid ${theme.border}` }}>
          ...
          {[["API Server", true], ["Database", true], ["Payment Gateway", true], ["File Storage", true], ["Email Service", true]].map(([name, ok]) => (
            <div key={String(name)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: `1px solid ${theme.border}` }}>
              <span style={{ fontSize: 13, color: theme.textSub }}>{String(name)}</span>
              <Badge label={ok ? "● Operational" : "● Degraded"} color={ok ? theme.success : theme.danger} bg={ok ? "rgba(34,211,165,0.1)" : "rgba(255,92,92,0.1)"} />
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <PageHeader title="Student Accounts" sub={`${students.length} registered students`} theme={theme} inline />
      ...
      <div style={{ background: theme.card, borderRadius: 16, border: `1px solid ${theme.border}`, overflow: "hidden" }}>
        ...
        {filtered.length === 0 ? <EmptyState icon="🎓" text="No students registered yet" /> : filtered.map(s => (
          <div key={s.id} style={{ display: "grid", gridTemplateColumns: "1.5fr 2fr 1.5fr 1fr 1fr 80px", padding: "13px 20px", borderBottom: `1px solid ${theme.border}`, alignItems: "center" }}>
            ...
            <Badge label={s.plan} color={s.plan === "premium" ? BLACK.warning : theme.accent} bg={s.plan === "premium" ? "rgba(240,160,32,0.12)" : theme.accentLight} />
            <button onClick={() => remove(s.id)} style={{ background: "rgba(255,92,92,0.1)", color: theme.danger, border: "none", borderRadius: 8, padding: "5px 10px", fontSize: 11, cursor: "pointer", fontFamily: F.body }}>Delete</button>
          </div>
        ))}
      </div>
    </div>
...
function AdminTutors({ tutors, setTutors, theme }: { tutors: TutorProfile[]; setTutors: React.Dispatch<React.SetStateAction<TutorProfile[]>>; theme: AnyTheme }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <PageHeader title="Tutor Accounts" sub={`${tutors.length} registered tutors`} theme={theme} inline />
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 10, padding: "8px 14px" }}>
          <span>🔍</span>
      ...
      <div style={{ background: theme.card, borderRadius: 16, border: `1px solid ${theme.border}`, overflow: "hidden" }}>
        ...
        {filtered.length === 0 ? <EmptyState icon="👨‍🏫" text="No tutors registered yet" /> : filtered.map(t => (
          <div key={t.id} style={{ display: "grid", gridTemplateColumns: "1.5fr 1.5fr 1fr 1fr 1fr 0.8fr 120px", padding: "13px 20px", borderBottom: `1px solid ${theme.border}`, alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}><Avatar initials={t.avatar} size={30} gradient={BLACK.tutor.gradient} /><span style={{ fontSize: 13, fontWeight: 600, color: theme.text }}>{t.name}</span></div>
            ...
            <Badge label={t.available ? "Available" : "Busy"} color={t.available ? theme.success : theme.danger} bg={t.available ? "rgba(34,211,165,0.1)" : "rgba(255,92,92,0.1)"} />
            <Badge label={t.verified} color={t.verified === "approved" ? theme.success : t.verified === "pending" ? theme.warning : theme.danger} bg={t.verified === "approved" ? "rgba(34,211,165,0.1)" : t.verified === "pending" ? "rgba(240,160,32,0.1)" : "rgba(255,92,92,0.1)"} />
            <div style={{ display: "flex", gap: 6 }}>
              ...
              <button onClick={() => remove(t.id)} style={{ background: "rgba(255,92,92,0.1)", color: theme.danger, border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, cursor: "pointer", fontFamily: F.body }}>Delete</button>
            </div>
          </div>
        ))}
      </div>
...
function AdminVerification({ tutors, setTutors, theme }: { tutors: TutorProfile[]; setTutors: React.Dispatch<React.SetStateAction<TutorProfile[]>>; theme: AnyTheme }) {
  const approve = async (id: string) => {
    await supabase.from("tutor_profiles").update({ verified: "approved" }).eq("id", id);
    setTutors(p => p.map(t => t.id === id ? { ...t, verified: "approved" as VerifStatus } : t));
    try { const tutor = tutors.find(t => t.id === id); if (tutor) await fetch("/api/email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "tutor_verification", tutorEmail: tutor.email || "", tutorName: tutor.name, approved: true }) }); } catch { }
  };
  const reject = async (id: string) => {
    await supabase.from("tutor_profiles").update({ verified: "rejected" }).eq("id", id);
    setTutors(p => p.map(t => t.id === id ? { ...t, verified: "rejected" as VerifStatus } : t));
    try { const tutor = tutors.find(t => t.id === id); if (tutor) await fetch("/api/email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "tutor_verification", tutorEmail: tutor.email || "", tutorName: tutor.name, approved: false }) }); } catch { }
  };
  const pending = tutors.filter(t => t.verified === "pending");
  const reviewed = tutors.filter(t => t.verified !== "pending");
  return (
    <div>
      <PageHeader title="Tutor Verification" sub="Review and approve tutor applications" theme={theme} />
      ...
      {pending.length === 0 ? <div style={{ background: theme.card, borderRadius: 16, border: `1px solid ${theme.border}`, marginBottom: 24 }}><EmptyState icon="✅" text="All caught up! No pending verifications" /></div> : (
        <div style={{ marginBottom: 28 }}>
          {pending.map(t => (
            <div key={t.id} style={{ background: theme.card, borderRadius: 16, padding: 22, border: `1px solid ${theme.warning}44`, marginBottom: 12 }}>
              <div style={{ display: "flex", gap: 16, alignItems: "flex-start", marginBottom: 14 }}>
                <Avatar initials={t.avatar} size={52} gradient={BLACK.tutor.gradient} />
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                    {t.modules.map(s => <span key={s} style={{ background: theme.accentLight, color: theme.accent, fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 20 }}>{s}</span>)}
                  </div>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => approve(t.id)} style={{ background: "rgba(34,211,165,0.12)", color: theme.success, border: "none", borderRadius: 10, padding: "9px 22px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: F.body }}>✓ Approve</button>
                <button onClick={() => reject(t.id)} style={{ background: "rgba(255,92,92,0.1)", color: theme.danger, border: "none", borderRadius: 10, padding: "9px 22px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: F.body }}>✕ Reject</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <>
          ...
          {reviewed.map(t => (
            <div key={t.id} style={{ background: theme.card, borderRadius: 14, padding: "16px 20px", border: `1px solid ${theme.border}`, marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <Avatar initials={t.avatar} size={36} gradient={BLACK.tutor.gradient} />
              ...
              <Badge label={t.verified} color={t.verified === "approved" ? theme.success : theme.danger} bg={t.verified === "approved" ? "rgba(34,211,165,0.1)" : "rgba(255,92,92,0.1)"} />
            </div>
          ))}
        </>
      )}
...
function AdminPayments({ payments, bookings, theme }: { payments: Payment[]; bookings: Booking[]; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="Payment Monitoring" sub="Track all transactions and revenue" theme={theme} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(180px,1fr))", gap: 14, marginBottom: 24 }}>
        {[
...
function AdminReviews({ reviews, setReviews, theme }: { reviews: Review[]; setReviews: React.Dispatch<React.SetStateAction<Review[]>>; theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="Reviews & Ratings" sub="Monitor and moderate all platform reviews" theme={theme} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(160px,1fr))", gap: 14, marginBottom: 24 }}>
        {[
          { label: "Avg Rating", value: avg, icon: "⭐", color: theme.warning },
          { label: "Total Reviews", value: reviews.length, icon: "📝", color: theme.accent },
          { label: "Flagged", value: reviews.filter(r => r.flagged).length, icon: "🚩", color: theme.danger },
          { label: "Hidden", value: reviews.filter(r => r.hidden).length, icon: "🙈", color: theme.textSub },
        ].map(s => <StatCard key={s.label} label={s.label} value={s.value} icon={s.icon} color={s.color} theme={theme} />)}
      </div>
      <div style={{ background: theme.card, borderRadius: 16, border: `1px solid ${theme.border}`, overflow: "hidden" }}>
        ...
        {visible.length === 0 ? <EmptyState icon="⭐" text="No reviews found" /> : visible.map(r => (
          <div key={r.id} style={{ padding: "16px 20px", borderBottom: `1px solid ${theme.border}`, opacity: r.hidden ? 0.5 : 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6, flexWrap: "wrap" }}>
                  ...
                  {r.flagged && <Badge label="Flagged" color={theme.danger} bg="rgba(255,92,92,0.1)" />}
                  {r.hidden && <Badge label="Hidden" color={theme.textMuted} bg="rgba(255,255,255,0.05)" />}
                </div>
                <p style={{ margin: 0, fontSize: 13, color: theme.textSub, lineHeight: 1.5 }}>{r.comment}</p>
              </div>
              <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                <button onClick={() => toggleFlag(r.id)} style={{ background: r.flagged ? "rgba(255,92,92,0.1)" : theme.accentLight, color: r.flagged ? theme.danger : theme.accent, border: "none", borderRadius: 6, padding: "4px 10px", fontSize: 11, cursor: "pointer", fontFamily: F.body }}>{r.flagged ? "Unflag" : "Flag"}</button>
                <button onClick={() => toggleHide(r.id)} style={{ background: "rgba(255,255,255,0.05)", color: theme.textSub, border: "none", borderRadius: 6, padding: "4px 10px", fontSize: 11, cursor: "pointer", fontFamily: F.body }}>{r.hidden ? "Restore" : "Hide"}</button>
                <button onClick={() => del(r.id)} style={{ background: "rgba(255,92,92,0.1)", color: theme.danger, border: "none", borderRadius: 6, padding: "4px 10px", fontSize: 11, cursor: "pointer", fontFamily: F.body }}>Delete</button>
              </div>
            </div>
          </div>
        ))}
...
function AdminMaintenance({ theme }: { theme: AnyTheme }) {
  ...
  return (
    <div>
      <PageHeader title="System Maintenance" sub="Platform settings, diagnostics and controls" theme={theme} />
      <div style={{ background: theme.card, borderRadius: 16, border: `1px solid ${theme.border}`, overflow: "hidden", marginBottom: 20 }}>
        ...
        {items.map(item => (
          <div key={item.key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", borderBottom: `1px solid ${theme.border}` }}>
            ...
            <div onClick={() => toggle(item.key)} style={{ width: 48, height: 26, borderRadius: 13, background: settings[item.key] ? (item.danger ? theme.danger : theme.success) : "rgba(255,255,255,0.1)", cursor: "pointer", position: "relative", transition: "background 0.2s", flexShrink: 0 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div style={{ background: theme.card, borderRadius: 16, padding: 24, border: `1px solid ${theme.border}` }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {["Clear System Cache", "Backup Database", "Export Reports (CSV)", "Send Test Email"].map(a => (
              <button key={a} onClick={() => action(a)} style={{ padding: "10px 16px", borderRadius: 8, border: `1px solid ${theme.border}`, background: "rgba(255,255,255,0.03)", color: theme.textSub, fontSize: 13, cursor: "pointer", fontFamily: F.body, textAlign: "left", transition: "background 0.15s" }}
        ...
        <div style={{ background: theme.card, borderRadius: 16, padding: 24, border: `1px solid ${theme.border}` }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <button onClick={() => window.confirm("Purge all reviews?") && alert("Reviews purged.")} style={{ padding: "10px 16px", borderRadius: 8, border: `1px solid rgba(255,92,92,0.25)`, background: "rgba(255,92,92,0.08)", color: theme.danger, fontSize: 13, cursor: "pointer", fontFamily: F.body, textAlign: "left" }}>🗑 Purge All Reviews</button>
            <button onClick={() => window.confirm("Reset the entire platform?") && alert("Platform reset.")} style={{ padding: "10px 16px", borderRadius: 8, border: `1px solid rgba(255,92,92,0.25)`, background: "rgba(255,92,92,0.08)", color: theme.danger, fontSize: 13, cursor: "pointer", fontFamily: F.body, textAlign: "left" }}>⚠ Reset Platform Data</button>
          </div>
        </div>
      </div>
    </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
function PageHeader({ title, sub, theme, inline = false }: { title: string; sub: string; theme: AnyTheme; tutor?: boolean; inline?: boolean }) {
  if (inline) return (
    <div>
      <h1 style={{ fontFamily: F.display, fontSize: 28, fontWeight: 900, color: theme.text, margin: "0 0 4px", letterSpacing: "-0.02em" }}>{title}</h1>
      <p style={{ color: theme.textSub, fontSize: 14, margin: 0 }}>{sub}</p>
    </div>
  );
  return (
    <div style={{ marginBottom: 28 }}>
...
function StatCard({ label, value, icon, color, theme }: { label: string; value: string | number; icon: string; color: string; theme: AnyTheme }) {
  return (
    <div style={{ background: theme.card, borderRadius: 16, padding: "22px 20px", border: `1px solid ${theme.border}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <p style={{ margin: "0 0 8px", fontSize: 11, color: theme.textMuted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</p>
          <p style={{ margin: 0, fontSize: 30, fontWeight: 700, color: theme.text, fontFamily: F.mono }}>{value}</p>
        </div>
        <div style={{ width: 40, height: 40, borderRadius: 12, background: `${color}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>{icon}</div>
      </div>
    </div>
  );
}
...
function EmptyState({ icon, text }: { icon: string; text: string }) {
  return (
    <div style={{ textAlign: "center", padding: "48px 20px" }}>
      ...
      <p style={{ color: BLACK.textMuted, fontSize: 14, margin: 0, fontFamily: F.body }}>{text}</p>
    </div>
  );
}
...
function StatusBadge({ status }: { status: BookingStatus }) {
  const config: Record<BookingStatus, { label: string; color: string; bg: string }> = {
    pending: { label: "Pending", color: BLACK.warning, bg: "rgba(240,160,32,0.12)" },
    confirmed: { label: "Confirmed", color: "#60A5FA", bg: "rgba(59,130,246,0.12)" },
    completed: { label: "Completed", color: BLACK.success, bg: "rgba(34,211,165,0.12)" },
    cancelled: { label: "Cancelled", color: BLACK.danger, bg: "rgba(255,92,92,0.12)" },
  };
  const c = config[status];
  return <Badge label={c.label} color={c.color} bg={c.bg} />;
}

function StatusPayBadge({ status }: { status: "completed" | "pending" | "refunded" }) {
  const config = {
    completed: { color: BLACK.success, bg: "rgba(34,211,165,0.12)" },
    pending: { color: BLACK.warning, bg: "rgba(240,160,32,0.12)" },
    refunded: { color: BLACK.danger, bg: "rgba(255,92,92,0.12)" },
  };
  const c = config[status];
...
// FILE UPLOAD WIDGET
// ═══════════════════════════════════════════════════════════════════════════════
function FileUploadWidget({ user, theme, onUploaded }: { user: User; theme: AnyTheme; onUploaded: (url: string, fileName: string, pages: number) => void; }) {
  ...
  const handleFile = async (file: File) => {
    ...
    if (!allowed.includes(file.type) && !file.name.endsWith(".pdf") && !file.name.endsWith(".docx")) { setError("Only PDF and Word documents are allowed."); return; }
  ...
  return (
    <div>
      ...
      <div onClick={() => fileRef.current?.click()} onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={e => { e.preventDefault(); setDragging(false); const f = e.dataTransfer.files[0]; if (f) handleFile(f); }}
        style={{ border: `2px dashed ${dragging ? theme.accent : theme.border}`, borderRadius: 14, padding: "28px 20px", textAlign: "center", cursor: "pointer", background: dragging ? theme.accentLight : "transparent", transition: "all 0.2s" }}>
        {uploading ? (
          <div>
            <p style={{ color: theme.textSub, fontSize: 14, fontFamily: F.body, margin: "0 0 12px" }}>Uploading... {progress}%</p>
            <div style={{ width: "100%", height: 6, background: theme.border, borderRadius: 3, overflow: "hidden" }}>
              <div style={{ width: `${progress}%`, height: "100%", background: theme.gradient, transition: "width 0.3s", borderRadius: 3 }} />
            </div>
          </div>
        ) : (
          <div>
            <svg width="32" height="32" fill="none" stroke={theme.accent} strokeWidth="1.5" viewBox="0 0 24 24" style={{ marginBottom: 10 }}>
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" y1="3" x2="12" y2="15" />
            </svg>
            <p style={{ color: theme.text, fontSize: 14, fontWeight: 600, margin: "0 0 4px", fontFamily: F.body }}>Drag & drop or <span style={{ color: theme.accent }}>click to upload</span></p>
            <p style={{ color: theme.textMuted, fontSize: 12, margin: 0, fontFamily: F.body }}>PDF or Word · Max 50MB</p>
          </div>
        )}
      </div>
      {error && <p style={{ color: BLACK.danger, fontSize: 12, margin: "8px 0 0", fontFamily: F.body }}>{error}</p>}
    </div>
  );
}
...
function MaterialUploadForm({ user, theme, onClose }: { user: User; theme: AnyTheme; onClose: () => void; }) {
  ...
  const inp: React.CSSProperties = { width: "100%", padding: "11px 14px", borderRadius: 10, border: `1px solid ${theme.border}`, background: "rgba(255,255,255,0.04)", color: theme.text, fontSize: 14, fontFamily: F.body, outline: "none", boxSizing: "border-box" };
  const lbl: React.CSSProperties = { display: "block", fontSize: 11, fontWeight: 600, color: theme.textSub, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.07em", fontFamily: F.body };
  ...
  if (done) return (
    <div style={{ background: theme.card, borderRadius: 16, padding: 32, border: `1px solid ${theme.border}`, textAlign: "center" }}>
      <div style={{ width: 56, height: 56, borderRadius: 16, background: "rgba(34,211,165,0.12)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 24 }}>✓</div>
      <h3 style={{ fontFamily: F.display, fontSize: 20, color: theme.text, margin: "0 0 8px" }}>Material Uploaded!</h3>
      <p style={{ color: theme.textSub, fontSize: 14, margin: "0 0 20px", fontFamily: F.body }}>Your material is now visible to students.</p>
      <button onClick={onClose} style={{ background: theme.gradient, color: "#fff", border: "none", borderRadius: 10, padding: "10px 24px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: F.body }}>Done</button>
    </div>
  );

  return (
    <div style={{ background: theme.card, borderRadius: 16, padding: 28, border: `1px solid ${theme.accent}33` }}>
      <h3 style={{ fontFamily: F.display, fontSize: 20, color: theme.text, margin: "0 0 22px" }}>Upload Study Material</h3>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div onClick={() => setPremium(!premium)} style={{ width: 44, height: 24, borderRadius: 12, background: premium ? theme.accent : "rgba(255,255,255,0.1)", cursor: "pointer", position: "relative", transition: "background 0.2s", flexShrink: 0 }}>
          ...
          <span style={{ fontSize: 13, color: theme.textSub, fontFamily: F.body }}>Mark as Premium (requires subscription)</span>
        </div>
        <div>
          <label style={lbl}>Upload File *</label>
          {fileUrl ? (
            <div style={{ background: "rgba(34,211,165,0.08)", border: "1px solid rgba(34,211,165,0.25)", borderRadius: 10, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <p style={{ margin: 0, fontSize: 13, color: theme.success, fontFamily: F.body, fontWeight: 600 }}>{fileName}</p>
              <button onClick={() => { setFileUrl(""); setFileName(""); }} style={{ background: "none", border: "none", color: theme.danger, cursor: "pointer", fontSize: 12, fontFamily: F.body }}>Remove</button>
            </div>
          ) : (
            <FileUploadWidget user={user} theme={theme} onUploaded={(url, name, pg) => { setFileUrl(url); setFileName(name); setPages(pg); }} />
          )}
        </div>
        {err && <p style={{ color: theme.danger, fontSize: 13, margin: 0, fontFamily: F.body }}>{err}</p>}
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={submit} disabled={submitting} style={{ background: submitting ? "rgba(255,255,255,0.1)" : theme.gradient, color: "#fff", border: "none", borderRadius: 10, padding: "11px 24px", fontSize: 14, fontWeight: 700, cursor: submitting ? "not-allowed" : "pointer", fontFamily: F.body }}>
            {submitting ? "Uploading..." : "Submit Material"}
          </button>
          <button onClick={onClose} style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${theme.border}`, color: theme.textSub, borderRadius: 10, padding: "11px 20px", fontSize: 14, cursor: "pointer", fontFamily: F.body }}>Cancel</button>
        </div>
      </div>
    </div>
  );
...
function ReferralWidget({ student, theme }: { student: StudentProfile; theme: AnyTheme }) {
  ...
  const referralLink = typeof window !== "undefined" ? `${window.location.origin}?ref=${student.referralCode || ""}` : `https://KitsoLink.vercel.app?ref=${student.referralCode || ""}`;
  const copy = () => { navigator.clipboard.writeText(referralLink).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }); };
  return (
    <div style={{ background: theme.card, borderRadius: 16, padding: 24, border: `1px solid ${theme.border}`, maxWidth: 640 }}>
      <h3 style={{ fontFamily: F.display, fontSize: 20, color: theme.text, margin: "0 0 6px" }}>Refer a Friend</h3>
      <p style={{ color: theme.textSub, fontSize: 14, margin: "0 0 20px", fontFamily: F.body, lineHeight: 1.6 }}>Share your link. When a friend signs up using it, you both get <strong style={{ color: theme.text }}>7 free days of Premium</strong>!</p>
      <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${theme.border}`, borderRadius: 10, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
        <p style={{ margin: 0, fontSize: 13, color: theme.textSub, fontFamily: F.mono, wordBreak: "break-all" }}>{referralLink}</p>
        <button onClick={copy} style={{ background: copied ? "rgba(34,211,165,0.12)" : theme.gradient, color: copied ? theme.success : "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: F.body, flexShrink: 0 }}>
          {copied ? "Copied! ✓" : "Copy Link"}
        </button>
      </div>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <span style={{ background: theme.accentLight, color: theme.accent, fontSize: 11, fontWeight: 600, padding: "4px 12px", borderRadius: 20, fontFamily: F.body }}>Your Code: {student.referralCode || "Loading..."}</span>
        <span style={{ background: "rgba(59,130,246,0.1)", color: "#60A5FA", fontSize: 11, fontWeight: 600, padding: "4px 12px", borderRadius: 20, fontFamily: F.body }}>Reward: 7 Days Premium per referral</span>
      </div>
    </div>
  );
}